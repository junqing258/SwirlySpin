...
export class DButton extends Laya.Sprite {
	...
	handleTouchIn() {
        ...
        this.__clicked = true;
		this.filters = [this.holdFilter||lightFilter(0.60)];
	...
	on(type,caller,listener,args) {
		let self = this,
		_listener = function() {
			if (self._isForbid) return;
			listener.apply(caller||self, arguments);
		};
...
export function DButtonDecorator(button, props) {
	...
	props.aduio!==null && (_this.aduio = props.aduio || AD_BTN);
	_this.delay = props.delay || 300;
	_this.on = (type,caller,listener,args) => {
		return DButton.prototype.on.call(_this,type,caller,listener,args);
	};
	...
	Laya.getset(false, _this, "isForbid", ()=> _this._isForbid, val=> {
		_this._isForbid = val;
		_this.filters = _this._isForbid? [_this.holdFilter||lightFilter(0.60)] : null;
	});
