...
Laya.Timer.prototype.from = function(array, delay, caller, callback)  {
	let i = 0, self = this, onComplete = function() {};
	let cb = function() {
		if (i>=array.length) {
			onComplete();
			return self.clear(caller, cb);
		}
		callback.call(caller, array[i++], i);
	}
	self.loop(delay,caller,cb);
...
Laya.Timer.prototype.of = function(len, delay, caller, callback)  {
	let i = 0, self = this, onComplete = function() {};
	let cb = function() {
		if (i>=len) {
			onComplete();
...
Laya.Monitor = function() {
	...
	let _isActive = true, _blurTime, _monitorNum, _checkTimer;

	let _stageOnBlur = function() {
		if (!_isActive) return;
		_isActive = false;
		_monitorNum = 0;
