...
export default class SenseManager {
    ...
    static load(router, payload, cb, passivity) {
        let index = routerList.indexOf(router);
        if (index===-1) return console.warn(`${router} not registered`);
        if (!firstIn &&　(SenseManager.checkNext && !SenseManager.checkNext()) ) return;
        ...
        if (promiselist.length>0) {
            ...
            Promise.all(promiselist).then(value=> {
                this.changeSense(router, CurComponent.hasHased, cb, payload);
    ...
    static changeSense(router, hasHased, cb, payload) {
        let index = routerList.indexOf(router);
        ...
        if (curRouter) {
            ...
            let  _curSense = curSense;
            (payload && payload.destroylater)? 
                Laya.timer.once(payload.destroylater, null, ()=>  _curSense.destroy(true) ) :
                _curSense.destroy(true);
        } else {
            curRouter = router;
            curSense = preSense;
            curSense.actived = true;
...
/*window.addEventListener("hashchange", function() {
    ...
    if (hash!==curRouter) {
        SenseManager.load(hash.slice(1), null, null, true);
    }
});*/